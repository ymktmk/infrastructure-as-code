...
resource "aws_s3_bucket" "encer_images" {
  bucket = "encer-${var.env_name}-images"
  versioning {
    enabled = false
  }
}
...
# logging
resource "aws_s3_bucket" "encer_server_logs" {
  bucket = "${var.encer_server}-${var.env_name}-logs"
  acl    = "private"
...
resource "aws_s3_bucket_public_access_block" "encer_server_logs" {
  bucket                  = aws_s3_bucket.encer_server_logs.id
